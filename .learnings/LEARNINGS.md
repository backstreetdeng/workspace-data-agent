...
## [LRN-20260629-002] knowledge_gap
**Logged**: 2026-06-29T10:57:52+08:00
**Priority**: critical
**Status**: resolved
**Area**: data-pipeline/sql-schema

### Summary
tools/targeted_sql_pack.py 长期使用虚构的英文列名（sales_month, company_name, model_name, sales），与真实 PostgreSQL schema（销售日期, 企业名称, 通用名称, 销量）完全不一致，导致 E2E 必失败。

### Details
- **触发**: 老大在 git 恢复后让我做工作空间全面扫描；扫到 E2E 时 `run_targeted_sql_pack()` 立即抛 `column "sales_month" does not exist / UndefinedColumn`，blocks=[] / results=[]
- **真实 schema** (2026-06-29 在线验证):
  - `sales_import` 14 列：id, 销售日期 (integer YYYYMM), 产品型号, 企业名称, 产品商标, 技术类型, 乘用车细分, 通用名称, 整备质量, 总质量, 轴距, 车型级别, 销量
  - `tech_data` 49 列（含 通用名称 / 产品名称 / 企业名称 / 价格 / CLTC续驶里程 / 电池类型 / 电机型号 / 功率）
  - `config_data` 40 列（含 车型名称 / 厂商指导价 / 价格带 / 级别 / CLTC纯电续航里程 / 电动机总功率）
  - 销售日期范围 202501~202602（14 个月）
- **references/schemas/database-schema.md 也是错的**：写的是"月份/车型/价格"，这三列根本不存在于 sales_import；三表 schema 需要按真实列名重写
- **TOOLS.md 4 处函数名错位**：`search_chunks` → 实际 `search_by_intent` / `vector_search`；`search_web` → 实际 `tavily_search`；`nl2sql_pg.nl2sql` → 实际 `from nl2sql import nl_to_sql`；`cn_search` 缺 Python 入口

### Action Taken
- 老大提供 `targeted_sql_pack_new.py`（正确版本），替换原坏文件
- 验证 8 块全部返回：BYD 近6月 total_sales=1130047 (份额 19.02%)、理想 2025年 8 块、无 target_brand 4 块通用
- git commit `f1ac7b3` 已 push

### Remaining Issues (未修，需老大拍板)
1. `references/schemas/database-schema.md` 需按真实 schema 重写（与 MyVault 知识库可能有冲突，建议与大管家协同）
2. `TOOLS.md` 4 处函数名 + `run_targeted_sql_pack` 调用示例待修正
3. `price_and_config` 在 BYD 上 0 行：品牌映射给全名（"比亚迪汽车工业有限公司"），config_data.厂商 用短名（"比亚迪"），是真实数据缺口不是代码 bug——SOUL 原则 2 "缺口即信号"已正确标注

### Suggested Action
1. 任何 schema 假设必须先用 `information_schema.columns` 在线 verify 再写代码
2. AGENTS.md 增加 "schema 先 verify 再编码" 硬约束
3. TOOLS.md 与 references/ 文档需建立 CI 校验（`tests/test_schema_docs.py`）确保文档与 DB 真实 schema 一致
4. config_data 的品牌映射短表需要单独维护（与 sales_import 的企业名称映射分开）

### Metadata
- Source: user_feedback
- Related Files: tools/targeted_sql_pack.py, references/schemas/database-schema.md, TOOLS.md
- Tags: data-pipeline, sql-schema, knowledge_gap, postgresql, brand-mapping, critical-bug, v2.0-reconstruction
